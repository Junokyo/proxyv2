...
export default function UserPassSection() {
  ...
  const codeExamples: Record<string, string> = {
    curl: `curl -x "rotating.proxy.example.com:8080" \\
     -U "user_abc123:password123" \\
     "https://api.ipify.org?format=json"`,
    python: `import requests

proxy = {
    "http": "http://user_abc123:password123@rotating.proxy.example.com:8080",
    "https": "http://user_abc123:password123@rotating.proxy.example.com:8080"
}
...
const proxy = new HttpsProxyAgent('http://user_abc123:password123@rotating.proxy.example.com:8080');

axios.get('https://api.ipify.org?format=json', { httpsAgent: proxy })
  .then(response => console.log(response.data))
  .catch(error => console.error(error));`,
    php: `<?php
$proxy = "http://user_abc123:password123@rotating.proxy.example.com:8080";
    ...
    public static void main(String[] args) throws Exception {
        ...
        Authenticator.setDefault(new Authenticator() {
            protected PasswordAuthentication getPasswordAuthentication() {
                return new PasswordAuthentication("user_abc123", "password123".toCharArray());
            }
        });
...
class Program {
    static async Task Main() {
        var proxy = new WebProxy("http://rotating.proxy.example.com:8080") {
            Credentials = new NetworkCredential("user_abc123", "password123")
        };
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="mb-4 flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary/10">
            <Iconify icon="mdi:key" width={14} className="text-primary" />
          </div>
          <div>
            <h3 className="text-xs font-semibold text-foreground">User & Pass Auth</h3>
            <p className="text-[10px] text-muted-foreground">Configure with username/password</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <label className="mb-1 block text-[11px] text-muted-foreground">Session</label>
              <select
