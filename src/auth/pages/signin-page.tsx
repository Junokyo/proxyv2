...
export function SignInPage() {
  ...
  const { ready, authenticated, login } = useKC(); // <-- lấy từ Provider

  // ✅ Nếu đã SSO rồi thì rời trang login
  useEffect(() => {
    if (ready && authenticated) {
      navigate('/', { replace: true }); // hoặc paths.dashboard.root
    }
          ...
          >
            {isGoogleLoading ? (
              ...
              <>
                ...
                <Iconify icon="simple-icons:keycloak" />
                SSO in with Keycloak
              </>
            )}
          </Button>
        </div>
